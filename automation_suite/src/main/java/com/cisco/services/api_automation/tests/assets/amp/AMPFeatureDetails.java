...
@Feature("AMP Feature Details")
public class AMPFeatureDetails extends BeforeTestSuiteClassIT {
	...
	String apiKey="amp_feature_details";
	String endPoint;
	String table;

	@BeforeClass
	public void setup() {
		endPoint = AssetsData.ASSETS_AMP_GET_APIS.get(apiKey).getEndPointUrl();
		table = AssetsData.ASSETS_AMP_GET_APIS.get(apiKey).geteSIndex();

	}

	@Test(description = "/amp/v1/feature/details API 200 Response Validation")
	...
	@Test(dependsOnMethods = { "api200ResponseValidation" },description = "/amp/v1/feature/details Records Count Validation with DB")
	public void apiRecordsCountValidation( ) {
		...
		try { 
			apiRecordsCount=CommonTestAcrossAPIsIT.recordsCountSQL(apiKey, responseAPI);
